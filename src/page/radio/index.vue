	  ...
	  <ul class="nav nav-justified choose" data-name="title">
          ...
          <div data-name="con">
            <div class="moduleList" id = "height01" >
              <div class="singleM" v-for="item in deviceList.filter(item => item.switchStatus!=null)" :key="item.userID">
		            <div class="moduleStyle" :class="returnClass(item.deviceState,item.type)" @click.stop="itemClick($event, item)">
                  <div class="moduleNum" style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"><i :class="returnIcon(item.type)"  aria-hidden="true"></i>{{item.userID + " " + (item.name==null?"":item.name)}}
                    <span  :class="item.switchStatus.status === '0' ? 'on' : 'off'" @click.stop="switchControl(0,item.switchStatus)"> {{item.switchStatus.status === '0' ? '关闭' : '开启'}}</span>
                    <!-- <span  :class={} @click="switchControl(0,item.switchStatus)"> {{item.switchStatus.status ? '关闭' : '开启'}} </span> -->
                  </div>
		                <div class="moduleKind">{{returnType(item.type)}}
                        <i class="fa fa-user再" v-if = "item.deviceState=='active' || item.deviceState=='ringing'"></i>
        ...
        <div class="functionMenu">
          <ul class="nav nav-justified menuList">
            <li id="a1" @click="openSwitch(true)" @mousedown="$btnMousedown" @mouseup="$btnMouseup" @touchend = "$btnMouseup" @touchstart = "$btnMousedown">
              <i class="fa fa-fire fa-2x" aria-hidden="true"></i>
              <span>开</span>
            </li>
            <li id="a1" @click="openSwitch(false)" @mousedown="$btnMousedown" @mouseup="$btnMouseup" @touchend = "$btnMouseup" @touchstart = "$btnMousedown">
              <i class="fa fa-fire fa-2x" aria-hidden="true"></i>
              <span>关</span>
            </li>
...
<script>
  ...
  export default {
   ...
   async  created() {
    ...
    methods: {
    openSwitch(status){
      this.selectPhone.forEach((device, i)=>{
          let {ip,switchid} = device.switchStatus
            if(status=='0'){
              this.instance({method:'get',url:`switch/off/${ip}/${switchid}`})
            }else{
              this.instance({method:'get',url:`switch/on/${ip}/${switchid}`})
            }
      })
    },
    switchControl(switchid,switchStatus){
      let {ip} = switchStatus
